#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""범대륙 초국가 — 마도 공학 연맹 (Megacorps) → 캐논 변환.

범대륙 4번 범주 — 마도 공학 연맹, 9개 조직.

원본 상태:
- 군사/외교/역사/법률/경제: 각 파일에 1-2줄 유니크 개요 + 100% 동일 보일러플레이트 ## 2. 서사적 디테일
- 파벌/예하부대: 이름만 다르고 본문 100% identical 보일러 → 에이전트 재서술 필요
- 장소: ALL Wonder 보일러 stub → 완전 제외

처리 전략:
1. 장소 카테고리 스킵 (출력 없음)
2. ## 2. 서사적 디테일 (Epic Detail) 섹션 제거
3. 콜아웃 정밀 처리 (심층 아카이브·에픽 섭리·The Anchor 제거)
4. 파벌/예하부대는 stub 보존 → 후속 에이전트 재서술 대상
"""
import os, re, sys
...
CATEGORY = {
    "src":      "6. 범대륙 초국가 및 중립 세력 (Supranational & Neutral)/6-4. 마도 공학 연맹 (Megacorps)",
    "out":      "마도공학",
    "kr":       "마도 공학 연맹",
    "slug":     "megacorps",
    "overview": "00. 마도 공학 독점 기업 개요.md",
    "rel_prefix": "01-8. 세력 아카이브/6. 범대륙 초국가/6-4. 마도 공학 연맹",
    "aliases":  ["마도 공학 연맹", "Megacorps", "마도공학연맹"],
}

ORGS = [
    ("01. 에테르 마동기어 공방 (Aether Machina Forge)",        "에테르마동기어공방",   "에테르 마동기어 공방",   "aether-machina"),
    ("02. 아발론 선로 공단 (Avalon Transit Corporation)",      "아발론선로공단",        "아발론 선로 공단",       "avalon-transit"),
    ("03. 아스트라비아 항공 공단 (Astravia Aviation Corp)",    "아스트라비아항공공단",  "아스트라비아 항공 공단", "astravia-aviation"),
    ("04. 바벨 통신 중공업 (Babel Telecom Corp)",              "바벨통신중공업",        "바벨 통신 중공업",       "babel-telecom"),
    ("05. 아발론 철도 공사 (Avalon Railway Corp)",             "아발론철도공사",        "아발론 철도 공사",       "avalon-railway"),
    ("06. 아클레피우스 바이오텍 (Asclepius Biotech)",          "아클레피우스바이오텍",  "아클레피우스 바이오텍",  "asclepius-biotech"),
    ("07. 골라이엇 거신 주조창 (Goliath Titan Forges)",        "골라이엇거신주조창",    "골라이엇 거신 주조창",   "goliath-titan"),
    ("08. 벌칸 무스펠하임 마도공창 (Vulcan Muspelheim Foundry)", "벌칸무스펠하임마도공창", "벌칸 무스펠하임 마도공창", "vulcan-muspelheim"),
    ("09. 일루미나티 동력 발전소 (Illuminati Power Plant)",    "일루미나티동력발전소",  "일루미나티 동력 발전소", "illuminati-power"),
]

CATEGORIES = [
    # ("1. 주요 장소 (Locations)", "장소.md", "주요 장소", "locations"),  # 전체 Wonder 보일러 → 생략
...
def strip_epic_detail(text):
    """## 2. 서사적 디테일 (Epic Detail) 섹션을 통째로 제거한다."""
    lines = text.split("\n")
    out = []
    skip = False
    for ln in lines:
        if _EPIC_DETAIL_H.match(ln.strip()):
            ...
            continue
        if skip:
            # 다음 heading(# 으로 시작)이 나오면 skip 해제
...
for org_dir, org_out, org_kr, org_slug in ORGS:
    ...
    for sub, outname, label, prefix in CATEGORIES:
        ...
        # 장소 카테고리는 이미 CATEGORIES에서 주석 처리됨
        files = walk_md(d)
        if prefix == "factions":
