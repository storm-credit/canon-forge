#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""용의 후예 (Dragon Descendants) — 원문 보존 캐논 변환.

구조 특이사항:
  - 주요 장소: 도시·성지·요새·금단구역 (가문 없음)
  - 씨족(Clans): 가문 대신 씨족 사용, 관계도 포함
  - 군사 폴더: '2. 군사 (Military)' 한 폴더에 부대(01-08)·전략(0-1~0-5) 혼재
  - 암약조직: 폴더당 파일 1개
...
FACTION = {
    "src":        "2. 크림슨 대륙 (Crimson Continent)/3. 용의 후예 (Dragon Descendants)",
    "overview":   "용의 후예.md",
    "out":        "용의후예",
    ...
    "continent":  "crimson",
    "nav_region": "크림슨·중부",
    "rel_prefix": "01-8. 세력 아카이브/2. 크림슨 대륙/3. 용의 후예",
    "aliases":    ["용의 후예", "Dragon Descendants", "드래곤 후예", "용혈족"],
}
...
CLAN_DIR      = "3. 씨족 (Clans)"
CLAN_INDEX    = "3. 씨족 (Clans)/0. 용의 후예 세력 관계도.md"
MILITARY_DIR  = "2. 군사 (Military)"
...
# 3) 씨족 (Clans) — 개별 파일 -------------------------------------------------
clan_d = os.path.join(SRC_ROOT, CLAN_DIR)
if os.path.isdir(clan_d):
    for fname in sorted(os.listdir(clan_d)):
        if not fname.endswith(".md") or is_list_file(fname):
            continue
        src = os.path.join(clan_d, fname)
        kname = korean_name_from(fname)
        body = transform(src, kname)
        sl = slug_from(kname)
        fm = frontmatter(f"faction.{slug}.clan.{sl}", kname, "씨족", [src])
        created.append(write_out(os.path.join("씨족", sl + ".md"), fm, body))
        nav_entities["씨족"].append((kname, f"씨족/{sl}.md"))
        print(f"  [씨족] {kname}")

# 3-b) 씨족 관계도 개요 --------------------------------------------------------
ci = os.path.join(SRC_ROOT, CLAN_INDEX)
if os.path.exists(ci):
    body = transform(ci, f"{kr} 씨족")
    fm = frontmatter(f"faction.{slug}.clan.index", f"{kr} 씨족 관계도",
                     "씨족(개요)", [ci])
    created.append(write_out("씨족.md", fm, body))
    print("  [씨족 개요] 씨족.md")

# 4) 군사: 부대(01~08 접두사)·전략(0-1~0-5 접두사) 분리 ----------------------
mil_d = os.path.join(SRC_ROOT, MILITARY_DIR)
if os.path.isdir(mil_d):
    unit_files = []
    strat_files = []
    for fname in sorted(os.listdir(mil_d)):
        if not fname.endswith(".md"):
            continue
        # 0-N.* = 전략 문서, 0N.* = 부대 (01.~08.)
        if re.match(r"^0-\d", fname):
            strat_files.append(fname)
        elif re.match(r"^0\d\.", fname):
            unit_files.append(fname)
        # 그 외(00. 목록 등)는 제외

    # 4-a) 부대: 파일당 개별 캐논
    for fname in unit_files:
        src = os.path.join(mil_d, fname)
        kname = korean_name_from(fname)
        body = transform(src, kname)
        sl = slug_from(kname)
...
nav_order += [("씨족", "씨족"), ("부대", "군사 — 부대"), ("암약조직", "암약조직")]
for grp, label in nav_order:
    items = list(nav_entities[grp])
    if grp == "씨족" and os.path.exists(os.path.join(OUT_ROOT, "씨족.md")):
        items = [("씨족 관계도 (개요)", "씨족.md")] + items
    if not items:
        continue
    print(f"                  - {label}:")
