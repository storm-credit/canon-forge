...
# ── 변환할 세력 설정 (이번 실행 대상) ────────────────────────────────────────
FACTION = {
    "src":        "3. 프로스트 대륙 (Frost Continent)/1. 딥 포지 왕국 (Deep Forge Kingdom)",
    "overview":   "딥 포지 왕국 (Deep Forge Kingdom).md",
    "out":        "딥포지왕국",
    "slug":       "deep-forge-kingdom",
    "kr":         "딥 포지 왕국",
    "continent":  "frost",
    "nav_region": "프로스트·산악",
    "rel_prefix": "01-8. 세력 아카이브/3. 프로스트 대륙/1. 딥 포지 왕국",
    "aliases":    ["딥 포지 왕국", "Deep Forge Kingdom", "딥포지", "심연 대장간 왕국", "드워프 왕국"],
}
...
HOUSE_INDEX_SRC = "3. 가문 (Noble Houses)/0. 딥 포지 왕국 가문 관계도.md"
...
if os.path.exists(hi):
    body = transform(hi, f"{kr} 가문")
    fm = frontmatter(f"faction.{slug}.house.index", f"{kr} 가문 관계도",
                     "가문(개요)", [hi])
    created.append(write_out("가문.md", fm, body))
    print("  [가문 개요] 가문.md")
