...
# ── 변환할 세력 설정 (이번 실행 대상) ────────────────────────────────────────
FACTION = {
    "src":        "1. 에테르 대륙 (Ether Continent)/3. 자유도시연합 (Crossroad Cities)",
    "overview":   "자유도시연합 (Crossroad Cities).md",
    "out":        "자유도시연합",
    "slug":       "crossroad-cities",
    "kr":         "자유도시연합",
    "continent":  "ether",
    "nav_region": "에테르·교차로",
    "rel_prefix": "01-8. 세력 아카이브/1. 에테르 대륙/3. 자유도시연합",
    "aliases":    ["자유도시연합", "Crossroad Cities", "크로스로드 시티즈", "교차로 도시연합", "자유도시 동맹"],
}
...
ENTITY = [
    ...
    ("3. 세력 (Factions)",                "세력",     "세력",     "faction-group"),
]
HOUSE_INDEX_SRC = "3. 세력 (Factions)/0. 자유도시연합 세력 관계도.md"
...
if os.path.exists(hi):
    body = transform(hi, f"{kr} 세력")
    fm = frontmatter(f"faction.{slug}.faction-group.index", f"{kr} 세력 관계도",
                     "세력(개요)", [hi])
    created.append(write_out("세력.md", fm, body))
    print("  [세력 개요] 세력.md")
...
for grp, label in nav_order:
    ...
    if grp == "세력" and os.path.exists(os.path.join(OUT_ROOT, "세력.md")):
        items = [("세력 관계도 (개요)", "세력.md")] + items
    if not items:
        continue
    print(f"                  - {label}:")
